...
if [[ "$TEST_CONFIRM" =~ ^[Yy]$ ]]; then
  echo "🔍 Running test command: python -m idml2docbook hello_world.idml"

  if [ ! -f "hello_world.idml" ]; then
    echo "❌ Test file 'hello_world.idml' not found in current directory: $(pwd)"
    echo "Please add it before running the test."
    exit 1
  fi

  EXPECTED_OUTPUT='<?xml version=1.0 encoding=utf-8?><article version=5.0 xml:lang=fr-FR xmlns=http://docbook.org/ns/docbook><para role=NormalParagraphStyle><phrase>Hello world!</phrase></para></article>'

  ACTUAL_OUTPUT=$(python -m idml2docbook hello_world.idml 2>/dev/null | tr -d '\n' \
  | tr -d '\r' | xargs)

  if [ "$ACTUAL_OUTPUT" = "$EXPECTED_OUTPUT" ]; then
    echo "✅ Test passed! Setup is working correctly."
  else
